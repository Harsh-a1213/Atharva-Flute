import React from 'react';
import { motion } from 'framer-motion';
import { heroImage } from './assets';

interface HeroProps {
  openBookingModal: () => void;
  openEnrollModal: () => void;
  openPerformanceModal: () => void;
}

const Hero: React.FC<HeroProps> = ({
  openBookingModal,
  openEnrollModal,
}) => {
  return (
    <section
      id="hero"
      className="relative min-h-screen lg:min-h-[750px] flex items-center justify-center text-center overflow-hidden"
    >
      {/* Gradient Overlay */}
      <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/60 to-black/80 z-10" />

      {/* Background Image */}
      <img
        src={heroImage}
        alt="Atharva Flute Performance"
        className="absolute inset-0 w-full h-full object-cover object-top z-0"
      />

      {/* Floating Notes (Desktop only) */}
      <div className="absolute inset-0 z-10 pointer-events-none hidden md:block">
        {[...Array(14)].map((_, i) => (
          <motion.svg
            key={i}
            xmlns="http://www.w3.org/2000/svg"
            className="absolute w-5 h-5 text-yellow-400 opacity-60"
            fill="currentColor"
            viewBox="0 0 24 24"
            style={{
              top: `${Math.random() * 100}%`,
              left: `${Math.random() * 100}%`,
              animation: `floatUp ${12 + Math.random() * 8}s linear infinite`,
            }}
            initial={{ opacity: 0 }}
            animate={{ opacity: [0, 1, 0] }}
            transition={{ duration: 8, repeat: Infinity, delay: i * 0.4 }}
          >
            <path d="M9 17.5C9 18.9 7.9 20 6.5 20S4 18.9 4 17.5 5.1 15 6.5 15c.56 0 1.07.19 1.48.5V5l10-2v10.5c0 1.4-1.1 2.5-2.5 2.5s-2.5-1.1-2.5-2.5 1.1-2.5 2.5-2.5c.56 0 1.07.19 1.48.5V6.3L10 7.8v8.2c0 1.4-1.1 2.5-2.5 2.5z" />
          </motion.svg>
        ))}
      </div>

      {/* Content */}
      <motion.div
        className="relative z-20 px-6 max-w-5xl"
        initial={{ opacity: 0, y: 32 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 1.1, ease: 'easeOut' }}
      >
        <h1 className="text-4xl md:text-6xl font-serif font-semibold tracking-wide leading-tight text-white mb-6">
          Join the Best Online & Offline Instrumental Classes
        </h1>

        <p className="text-base md:text-xl text-brand-light max-w-3xl mx-auto leading-relaxed mb-10">
          Learn from a passionate flautist from Nashik and start your musical journey.
        </p>

        <div className="flex flex-col md:flex-row justify-center gap-6">
          <button
            onClick={openBookingModal}
            className="bg-brand-gold text-brand-dark font-semibold py-4 px-10 rounded-full text-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition"
          >
            Book a Free Trial
          </button>

          <button
            onClick={openEnrollModal}
            className="bg-brand-gold text-brand-dark font-semibold py-4 px-10 rounded-full text-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition"
          >
            Enroll Now
          </button>
        </div>
      </motion.div>

      <style>
        {`
          @keyframes floatUp {
            0% { transform: translateY(0); opacity: 0; }
            20% { opacity: 1; }
            100% { transform: translateY(-110vh); opacity: 0; }
          }
        `}
      </style>
    </section>
  );
};

export default Hero;
